...
public class UserServiceImpl implements UserService {
  private static final Pattern P_USER_NAME =
      Pattern.compile("^[0-9a-zA-Z_]{5,50}$");
  private static final Pattern P_USER_PHONE =
      Pattern.compile("^(13[0-9]|14[01456879]|15[0-35-9]|16[2567]|17[0-8]|18[" +
                      "0-9]|19[0-35-9])\\d{8}$");
  private static final Pattern P_USER_MAIL =
  ...
  public PagingData<UserVO> getUserPage(UserQueryDTO queryDTO) {
    ...
    if (queryDTO.getRoleId() != null &&
        CollectionUtils.isEmpty(userIdList =
                                    this.userRoleService.getUserIdListByRoleId(
                                        queryDTO.getRoleId()))) {
      return new PagingData<UserVO>((IPage<?>)new Page(
          (long)queryDTO.getPage(), (long)queryDTO.getSize()));
    }
    IPage<User> pageInfo =
    ...
    for (User user : userList) {
      ...
      userVo.setRoleList(userId2RoleListMap.getOrDefault(
      ...
      userVo.setProjectList(
          Lists.newArrayList((Iterable)userId2ProjectListMap.getOrDefault(
              user.getId(), Collections.emptySet())));
    }
  ...
  public PagingData<UserBriefVO> getUserBriefPage(UserBriefQueryDTO queryDTO) {
    ...
    IPage<UserBrief> pageInfo =
        this.userDao.selectBriefPageByDeptIdList(queryDTO, deptIdList);
    List<UserBriefVO> userBriefVOList =
        CopyBeanUtil.copyList(pageInfo.getRecords(), UserBriefVO.class);
  ...
  public UserVO getUserDetailByUserId(Integer userId)
    ...
    List<ProjectBriefVO> projectBriefVOS =
        this.userProjectDao
            .selectProjectIdListByUserIdList(
                Collections.singletonList(userVo.getId()))
            .stream()
            .map(this.projectDao::selectByProjectId)
  ...
  public Result<List<UserVO>> getUserDetailByUserIds(List<Integer> ids) {
    ...
    List userVOS = ids.stream()
                       .distinct()
                       .map(this.userDao::selectByUserId)
                       .filter(Objects::nonNull)
                       .map(user -> {
                         UserVO userVO = CopyBeanUtil.copy(user, UserVO.class);
                         userVO.setUpdateTime(user.getUpdateTime());
    ...
    Map<Integer, ProjectBriefVO> projectId2ProjectMap =
        projects.stream().collect(
            Collectors.toMap(ProjectBriefVO::getId, i -> i));
    Map userId2ProjectListMap =
        userProjectList.stream().collect(Collectors.groupingBy(
            UserProjectPO::getUserId,
            Collectors.mapping(
                i
                -> (ProjectBriefVO)projectId2ProjectMap.get(i.getProjectId()),
                Collectors.toSet())));
    for (UserVO userVO : userVOS) {
      ...
      List<Integer> hasPermissionIdList =
          this.rolePermissionService.getPermissionIdListByRoleIdList(
              roleIdList);
      userVO.setPermissionTreeVO(
          this.permissionService.buildPermissionTreeWithHas(
              hasPermissionIdList));
      userVO.setProjectList(Lists.newArrayList(
          (Iterable)userId2ProjectListMap.get(userVO.getId())));
    }
  ...
  getUserBriefListByUserIdList(List<Integer> userIdList) {
    ...
    List<UserBriefVO> userBriefVOS =
        CopyBeanUtil.copyList(userBriefList, UserBriefVO.class);
    for (UserBriefVO userBriefVO : userBriefVOS) {
      ...
      List<String> roles =
          this.roleService.getRoleBriefListByUserId(userBriefVO.getId())
              .stream()
  ...
  public List<UserBriefVO> getUserBriefListByUsernameOrRealName(String name) {
    List<UserBrief> userBriefList =
        this.userDao.selectBriefListByNameAndDescOrderByCreateTime(name);
  ...
  public List<UserBriefVO> getAllUserBriefListOrderByCreateTime(boolean isAsc) {
    List<UserBrief> userBriefList =
        this.userDao.selectBriefListOrderByCreateTime(isAsc);
    return CopyBeanUtil.copyList(userBriefList, UserBriefVO.class);
  }

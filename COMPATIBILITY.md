...
根 Maven reactor 以 Spring Boot 3.3.13 和 Java 21 为统一基线，当前只聚合
`yak-security`。该模块使用 Jakarta Servlet API 和面向 Spring Boot 3 的
MyBatis-Plus starter。

`yak-job` 暂时不在根 reactor 的 Boot 3 聚合构建边界内。它仍是一个遗留的

...
public final class InMemoryScheduleExecutionLogRepository implements ScheduleExecutionLogRepository {
    ...
    @Override
    public synchronized List<ScheduleExecutionLog> find(String project, String taskName) {
        return logs.stream().filter(log -> log.getProject().equals(project) && log.getTaskName().equals(taskName)).toList();
    }
}
